...
(function Shifty (global, __getMockNow) {
	...
	// This is just a testing hook to control the flow of time.
	if (__getMockNow) {
	  now = __getMockNow;
	}
	...
	/**
	 * CalcucurrentTimelates the interpolated tween values of an Object based on the current time.
	 * @param {Number} currentTime The current position to evaluate the tween against.
	...
	function tweenProps (currentTime, params, state) {
		...
		normalizedPosition = (currentTime - params.timestamp) / params.duration;
	...
	function runCurrentFrameLogic (params, state) {
	  var currentTime;
		...
		if (currentTime < params.timestamp + params.duration) {
			updateStateForCurrentFrame(currentTime, params, state);
		} else {
	 ...
	 *   @property {Tweenable} owner The `Tweenable` instance that the tween this function is acting upon belongs to.
	...
	function timeoutHandler (params, state) {
	  ...
	  runCurrentFrameLogic(params, state);
	}
	
	function updateStateForCurrentFrame (currentTime, params, state) {
	...
	Tweenable.prototype.tween = function tween (from, to, duration, callback, easing) {
		...
		if (this._state.isAnimating || this._state.isPaused) {
			return;
		}
